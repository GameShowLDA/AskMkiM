...
| `MeasurementMessages` | static facade | Ask.Protocol.Messages | принимает `MeasurementRange`, обозначение точки/цепи либо `Разряд N (binary)`, формирует и публикует итоговые и промежуточные результаты с отдельными настройками видимости | [Protocols](#protocols-and-file-formats) |
| `MeasurementMessageBuilder` | internal static builder | Ask.Protocol.Messages | формирует единый формат диапазона `(unit<max)`, `(min<unit<max)` или `(min<unit)`, измеренное значение, `ПРОБОЙ` и `Overload` | [Protocols](#protocols-and-file-formats) |
