...
| `CommandMessages` | static facade | Ask.Protocol.Messages | проверяет настройки видимости этапов, формирует и выводит сообщения команд, блоков проверки, цепей, точек и разрядов; модели наружу не возвращает | [Protocols](#protocols-and-file-formats) |
| `CommandMessageBuilder` | internal static builder | Ask.Protocol.Messages | содержит перенесённую из `ExecutorMessageBuilder` логику сообщений команд и блоков проверки | [Protocols](#protocols-and-file-formats) |
| `CommandMessagePublisher` | internal static publisher | Ask.Protocol.Messages | передаёт сформированные сообщения команд в `IMessageOutputService` с метаданными исходного вызова | [Protocols](#protocols-and-file-formats) |
...
| `MeasurementMessages` | static facade | Ask.Protocol.Messages | публикует начало измерения, этап измерений, ток утечки PI, эталонное значение, выдачу испытательного напряжения PI ACW/DCW, итоговые и промежуточные результаты и погрешности | [Protocols](#protocols-and-file-formats) |
...
| `ExecutionMessages` | static facade | Ask.Protocol.Messages | проверяет видимость параметров выполнения для настраиваемых сообщений и безусловно публикует границы этапов инициализации, настройки оборудования и запуска теста; модели наружу не возвращает | [Protocols](#protocols-and-file-formats) |
| `ExecutionMessageBuilder` | internal static builder | Ask.Protocol.Messages | содержит сообщения подготовки и настройки устройств, границ инициализации оборудования и запуска теста | [Protocols](#protocols-and-file-formats) |
| `ExecutionMessagePublisher` | internal static publisher | Ask.Protocol.Messages | передаёт сообщения этапов выполнения в `IMessageOutputService`, сохраняет признак начала блока и метаданные исходного вызова | [Protocols](#protocols-and-file-formats) |
| `ValidationMessages` | static facade | Ask.Protocol.Messages | публикует ошибки полей ввода и зависимостей самоконтроля, а также заголовок запуска и введённые параметры проверки | [Protocols](#protocols-and-file-formats) |
| `ValidationMessageBuilder` | internal static builder | Ask.Protocol.Messages | формирует ошибки данных, сообщения о зависимостях самоконтроля и представление введённых параметров запуска | [Protocols](#protocols-and-file-formats) |
