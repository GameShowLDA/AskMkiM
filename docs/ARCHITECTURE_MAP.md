...
- `MethodExecutor`, `NodeAccumulationChecker`, `NodeFullChecker` и `PairwiseFirstPointChecker`
  ...
  `PiCommandExecutor` использует её в промежуточном результате ACW/DCW, а исполнители
...
- `MeasurementMessages.PublishInsulationStrengthResultAsync` и
  ...
  Entry point используется узловыми и групповыми исполнителями PI ACW/DCW; измерения
  `PiCommandExecutor` публикуются отдельно как промежуточные числовые результаты;
