...
- `PairwiseFirstPointCheckerAlt` — специальная ЭТ-проверка; обходит все группы, цепи и точки,
  ...
  верхней границы, публикует заголовок блока локализации через `ExecutionMessages`, повторно измеряет
  такие точки, публикует каждое скомпенсированное измерение пары через
  `IMeasurementResultMessageExecutor` как промежуточный результат и рекурсивно разбивает цепь
  на связные фрагменты аналогично локализации ПР; перед каждой коммутацией, измерением и публикацией
  проверяет `CancellationToken` и `IExecutionPauseGate`, а обязательное снятие реле выполняет в
  `finally` без паузы; границы найденных разрывов сохраняет в итоговой строке как `*...**...*`,
  не используя разделитель `,,`; видимость строк измерений определяется
  `DeviceDisplayConfig.GetIntermediateMeasurementResultsVisibility`; если повтор не подтверждает
  точное разбиение, исходный верхний брак сохраняется для всей проверяемой цепи; результаты ниже
  нижней границы остаются обычными ошибками пары и не участвуют в разбиении; возвращает
