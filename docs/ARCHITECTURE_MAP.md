...
`ServiceUtilitiesControl` сохраняет экземпляры вложенных
  утилит при переключении;
    - `SetCommand` — отправка низкоуровневых команд и отображение общего потока
      ...
      карточки ручных операций и строки `LoggerUtility` добавляются в единый
      `ConsolePanel` в хронологическом порядке, без раздельных списков по типу;
      встроенная команда `ping IP` выполняет ICMP-проверку и показывает
      доступность, задержку и TTL в такой же карточке; команда `cmd COMMAND`
      запускает локальный `cmd.exe`, захватывает stdout/stderr и код завершения,
      ограничивает выполнение 30 секундами и также возвращает результат карточкой;
