...
у полосы — `IsViewportVisible="False"` и `AreToolTipsEnabled="False"`, поэтому видны
только маркеры без всплывающего предпросмотра, перекрывающего протокол при работе
со штатной вертикальной прокруткой.
...
клики работают над маркерами, свободная область пропускает мышь к скроллу.
