...
namespace Ask.Device.Application.FunctionAdapters.DeviceBusCommutation
{
  internal class CapacitorManagerAdapter : ICapacitorDeviceBusCommutation
  {
    ...
    public CapacitorManagerAdapter(Runtime.Device.SwitchingDevice.DeviceBusCommutation deviceBusCommutation)
    {
    ...
    public async Task<bool> ConnectCapacitor(int number, IUserInteractionService? userMessageService = null)
    {
      ...
      {
        var succes = await _capacitorManager.ConnectCapacitor(number, userMessageService);
        return succes;
    ...
    public async Task<bool> DisconnectCapacitor(int number, IUserInteractionService? userMessageService = null)
    {
      ...
      var result = await UserActionHelper.GetRunWithUserRepeatAsync(async () =>
      {
        var succes = await _capacitorManager.DisconnectCapacitor(number, userMessageService);
