...
namespace NewCore.FunctionAdapters.Keysight3466new
{
  ...
  internal class DcVoltageMeasurementAdapter : IDcVoltageMeasurement
  {
    ...
    public async Task SetDCVoltageModeAsync()
    {
      try
      {
        await _measurement.SetDCVoltageModeAsync();

        await DeviceMessageBuilder.ShowConnectionMessageAsync(
          _device,
          "Установка режима измерения постоянного напряжения",
          "CONF:VOLT:DC",
          true, 
          1);
      }
      catch (Exception ex)
      {
        await DeviceMessageBuilder.ShowConnectionMessageAsync(
          _device,
          "Ошибка при установке режима DC",
          ex.Message,
          false, 
          1);
        throw;
    ...
    public async Task<double> MeasureDCVoltageAsync(double param = 0)
    {
      ...
      catch (Exception ex)
      {
        await DeviceMessageBuilder.ShowConnectionMessageAsync(
          _device,
          "Ошибка при измерении DC-напряжения",
          ex.Message,
          false, 
          2);
        return -1;
