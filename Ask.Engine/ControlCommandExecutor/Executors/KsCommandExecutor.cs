...
namespace Ask.Engine.ControlCommandExecutor.Executors
{
  internal class KsCommandExecutor : CommandExecutorBase, ICommandExecutor
  {
    ...
    private async Task<(bool, double)> ResistanceMeasure(double value, IUserInteractionService messageService, CancellationToken cancellationToken, PointModel point, double errorResistance = 0)
    {
    ...
    /// </summary>
    /// <returns>Задача, представляющая измерение.</returns>
    private async Task<(bool, double)> FastResistanceMeasure(double value, IUserInteractionService messageService, CancellationToken cancellationToken, PointModel point, double errorResistance = 0)
    {
      var meter = await EquipmentService.GetFastMeterOrThrow(messageService);
      double answer = 0;
