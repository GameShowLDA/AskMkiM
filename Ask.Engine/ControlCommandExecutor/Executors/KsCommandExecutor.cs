...
{
  internal class KsCommandExecutor : CommandExecutorBase, ICommandExecutor, IMeasurementResultMessageExecutor
  {
    ...
    private async Task<(bool, double)> ResistanceMeasure(double value, IUserInteractionService messageService, CancellationToken cancellationToken, PointModel firstPoint, PointModel checkedPoint, double errorResistance = 0)
    {
      ...
      var result = await UserActionHelper.GetRunWithUserRepeatAsync(async () =>
      {
        ...
        var resultContext = new MeasurementResultMessageContext(
          MeasurementTypeCommand.KC, measurementRange, messageService);
