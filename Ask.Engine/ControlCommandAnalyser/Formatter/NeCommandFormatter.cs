...
namespace Ask.Engine.ControlCommandAnalyser.Formatter
{
  public class NeCommandFormatter : ICommandFormatter
  {
    ...
    public IEnumerable<string> Format(BaseCommandModel model)
    {
      ...
      {
        yield return "\tПротекание тока в прямом направлении:";
        ...
        for (int i = 0; i < ne.Scheme.GroupModels.Count; i++)
        {
          var points = ne.Scheme.GetPointsDisconnected(ne.Scheme.GroupModels[i]);
          if (points != null)
          {
            string str = string.Empty;
            str += $"\t\t\t{i + 1}. *{ne.ElementEnablingType[i].Item2.GetDescription()}";
