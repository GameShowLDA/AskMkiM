...
namespace UI.Controls.ProtocolController
{
  ...
  public class ActionExecutor
  {
    ...
    /// </summary>
    ProtocolController ProtocolSelfCheck;
    ...
    public static async Task<ActionExecutor> CreateInstanceAsync<T>(T parentClass)
    {
      try
      {
        if (parentClass != null)
        {
          if (parentClass.GetType() == typeof(ProtocolController))
          {
            return await DefaultSettings(parentClass as ProtocolController);
          }
        }
      }
      catch (Exception)
    ...
    /// <returns>Настроенный экземпляр <see cref="ActionExecutor"/>.</returns>
    static private async Task<ActionExecutor> DefaultSettings(ProtocolController parentClass)
    {
