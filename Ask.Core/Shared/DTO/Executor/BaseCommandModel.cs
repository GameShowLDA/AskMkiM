using Ask.Core.Services.Errors.Models;
using Ask.Core.Shared.Interfaces.ErrorInterfaces;
using Ask.Core.Shared.Interfaces.ExecutionInterfaces;
using Ask.Core.Shared.Metadata.Enums.TranslationEnums.Commands;

namespace Ask.Core.Shared.DTO.Executor
{
  /// <summary>
  /// Ѕазова€ модель любой команды после разбора.
  /// </summary>
  public abstract class BaseCommandModel : IError, IExecutionCommandInfo
  {
    public virtual MeasurementTypeCommand TypeCommand { get; set; }
    public List<string> SourceLines { get; set; } = new List<string>();

    public List<ErrorItem> Errors { get; set; } = new List<ErrorItem>();
    public List<WarningItem> Warnings { get; set; } = new List<WarningItem>();

    /// <summary>
    /// Ќомер строки, с которой начинаетс€ команда (в исходном тексте).
    /// </summary>
    public int StartLineNumber { get; set; }

    /// <summary>
    /// Ќомер строки, с которой начинаетс€ команда в отформатированном (трансл€ционном) тексте.
    /// ѕроставл€етс€ после форматировани€.
    /// </summary>
    public int FormattedStartLineNumber { get; set; } = -1;

    /// <summary>
    ///  лючи алгоритма проверки, указанные в команде.
    /// </summary>
    public List<string> AlgorithmKey { get; set; } = new();

    /// <summary>
    ///  омментарии, указанные в команде.
    /// </summary>
    public List<string> Comment { get; set; } = new();

    public virtual IPointError PointErrors => null;

    public virtual IDislpayInfo BuildDislpayInfo => null;

    public string CommandNumber { get; set; }
    public virtual string Mnemonic { get; set; }
    public string PointsSourse { get; set; }

    /// <summary>
    /// “екстовое тело команды в исходном виде.
    /// </summary>
    public virtual string CommandBody => SourceLines.Count == 0
      ? string.Empty
      : string.Join(Environment.NewLine, SourceLines);

    /// <summary>
    /// ѕризнак установленной точки останова дл€ данной команды.
    /// ”казывает, что выполнение анализа или обработки должно быть остановлено при достижении этой команды.
    /// </summary>
    public bool HasBreakpoint { get; set; }

    #region ћетоды
    public virtual T GetModel<T>(BaseCommandModel baseCommandModel) where T : class
    {
      return baseCommandModel as T;
    }

    #endregion

  }
}
