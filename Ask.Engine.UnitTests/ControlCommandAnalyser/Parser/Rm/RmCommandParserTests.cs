...
public class RmCommandParserTests : IDisposable
{
  ...
  {
    var model = ParseLines("30 РМ X1/1=1.1.1");

    AssertErrorCodes(model);
    Assert.Single(model.PointsMap);
    Assert.Equal("1.1.1", model.PointsMap["X1/1"]);
  ...
  {
    var model = ParseLines("30 РМ X1/1==S1=1.1.1");

    AssertErrorCodes(model);
    Assert.Equal("1.1.1", model.PointsMap["X1/1"]);
    Assert.Equal("1.1.1", model.SynonymMap["S1"]);
    Assert.True(model.TryGetAddressByKey("S1", out var address));
    Assert.Equal("1.1.1", address);
  ...
  public void Parse_WithSeveralBodyLines_KeepsFullSource()
  {
    ...
    AssertErrorCodes(model);
    Assert.Contains("X1/1=1.1.1", model.PointsSourse);
    Assert.Contains("X2/1=1.2.1", model.PointsSourse);
  }
  ...
  {
    var model = ParseLines("30 РМ A1-A3==S1-S3=1.1.1-1.1.3");

    AssertErrorCodes(model);
    Assert.Equal(new[] { "A1", "A2", "A3" }, model.PointsMap.Keys.ToArray());
    Assert.Equal(new[] { "S1", "S2", "S3" }, model.SynonymMap.Keys.ToArray());
    Assert.Equal(new[] { "1.1.1", "1.1.2", "1.1.3" }, model.PointsMap.Values.ToArray());
  }
  ...
  public void Format_WithPartsAndSynonyms_OutputsStructuredCommand()
  {
    ...
    Assert.Contains("\tX1/1 == S1 = 1.1.1", lines);
    Assert.Contains("\t* Ч=2", lines);
    Assert.Contains("\tX2/1 = 1.2.1", lines);
  }
