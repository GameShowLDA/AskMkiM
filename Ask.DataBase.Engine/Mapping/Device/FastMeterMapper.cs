...
/// <summary>
/// Маппер для преобразования между <see cref="IMultimeter"/> и <see cref="FastMeterDto"/>.
...
{
  /// <summary>
  /// Преобразует реализацию <see cref="IMultimeter"/> в DTO <see cref="FastMeterDto"/>.
  ...
  public static FastMeterDto ToDto(IMultimeter device)
  {
    ArgumentNullException.ThrowIfNull(device);

    var dto = ReflectionMapper.Map<IMultimeter, FastMeterDto>(device);
  ...
  /// <summary>
  /// Применяет данные из <see cref="FastMeterDto"/> к существующему объекту <see cref="IMultimeter"/>.
  ...
  public static void ApplyDto(IMultimeter device, FastMeterDto dto)
  {
    ArgumentNullException.ThrowIfNull(device);
