...
/// <summary>
/// Базовая реализация USB-мультиметра, работающего по SCPI.
/// </summary>
public abstract class MultiUsbSCPIMeterBase : DeviceWithUSB, IMultimeter
{
  /// <summary>
  /// Инициализирует общие менеджеры USB-мультиметра.
  /// </summary>
  protected MultiUsbSCPIMeterBase()
  {
  ...
  /// <summary>
  /// Заполняет основные параметры USB-мультиметра.
  /// </summary>
  protected void ConfigureUsbMeter(string name, string description, string usbPattern)
  {
    Name = name;
    Description = description;
    ConnectionDetails = usbPattern;
  }

  /// <summary>
  /// Настраивает команды SCPI, которые в старой системе использовались для Agilent-совместимых USB-мультиметров.
  /// </summary>
  protected void ConfigureAgilentUsbCommands(bool supportsCapacitance)
  {
