...
namespace Ask.Device.Runtime.Function.Base.Multimeter.Measurements
{
  internal class DiodeMeasurementBase : IDiodeMeasurement
  {
    ...
    /// <inheritdoc />
    public async Task<double> CheckDiodeAsync(double param = 0, double rangeFrom = -1, double rangeTo = -1, IUserInteractionService? userMessageService = null, double responseDelay = 0)
        => await MeasurementBase.MeasureAsync(_device, _device.DiodeCommands, param, rangeFrom, rangeTo, userMessageService, responseDelay: responseDelay);
  }
}
